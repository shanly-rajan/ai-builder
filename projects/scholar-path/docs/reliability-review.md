...
| 9 | Secret redaction in logs and traces | Trace metadata is scalar-allowlisted; graph and evaluation clients hide inputs and outputs; provider errors are sanitized; UI failures use generic messages. Queries, page content, Candidate research content, credentials, and thread IDs are excluded from trace metadata. | The CLI deliberately prints the Candidate review payload and opaque local thread ID. Operators must treat console output as local workflow data and must not redirect it to an unrestricted log sink. |
