...
def test_m13_3_prompt_active_documentation_and_journal_are_recorded() -> None:
    ...
    assert "Evidence-backed Supervisor discovery for Master's and doctoral research." in readme
    assert "## M13.3 academic presentation and research-degree scope" in architecture
...
def test_m13_3_active_prompts_cover_masters_and_doctoral_research_degrees() -> None:
    ...
    for prompt in (
        ...
        assert "Master's" in normalized
        assert "doctoral" in normalized
        assert "admission probability" in normalized
